...
- ~WidgetClass~ :: A factory which will create a widget by calling
     ~WidgetClass(*args)~ *(TODO what if ~**kwargs~ is in need?)*. It could
     also be a recognized string that corresponds to a known Qt class, e.g.,
     ~m('label')~ will be simply treated the same as ~m(QLabel)~.
- ~*args~ :: See above.
...
A list or tuple of cells will be wrapped in a container by default.
